...
function gameSubmit(plantToGuess) {
  ...
  answerText.forEach((span) => {
    span.classList.add('notAnswered');
  });

  // Check if radio button checked value is equal to plant to guess name
  if (answerInputChecked.value === plantToGuess['name']) {
    answerInputChecked.nextElementSibling.classList.add('rightAnswer');
    console.log('yes');
  } else {
    answerInputChecked.nextElementSibling.classList.add('wrongAnswer');
    answerText.forEach((span) => {
  ...
  submitButton.insertAdjacentHTML(
    'afterend',
    `<p class="game__description" id="plant-description" >${plantToGuess['description']}</p>`
  );
...
function reset() {
  let descriptionParagraph = document.getElementById('plant-description');
  const answerText = Array.from(
    document.getElementsByClassName('game__answers__item__text')
  );
  answerText.forEach((span) => {
    span.classList.remove('notAnswered');
    span.classList.remove('wrongAnswer');
    span.classList.remove('rightAnswer');
  });
